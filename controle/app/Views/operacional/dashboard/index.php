...
<div class="row g-3 mb-3">
  ...
  <div class="col-12 col-md-6">
    <div class="card position-relative">
      <div class="card-body">
        <h6 class="card-title mb-1">CMV</h6>
        <div class="d-flex justify-content-between">
          <div>
            <small class="text-muted">Total (Mês)</small>
            <div class="h4 mb-0">R$ <?= number_format((float)$cmvTotal, 2, ',', '.') ?></div>
          </div>
          <div>
            <small class="text-muted">CMV/PR</small>
            <div class="h4 mb-0">R$ <?= number_format((float)$cmvPr, 2, ',', '.') ?></div>
          </div>
        </div>
        <div class="text-muted small mt-2">
          Base: Almoço+Jantar = <?= number_format((int)$cmvRefBase, 0, ',', '.') ?>
        </div>
        <!-- link invisível que torna o card clicável -->
        <a href="<?= base_url('operacional/cmv') ?>" class="stretched-link" aria-label="Ver histórico do CMV"></a>
      </div>
    </div>
  </div>
</div>
