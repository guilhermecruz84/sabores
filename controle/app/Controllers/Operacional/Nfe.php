...
class Nfe extends OperacionalBase
{
    ...
    private const BASE = 'operacional/nfe';
    ...
    public function upload()
    {
        ...
        $countXML   = 0;
        $countItens = 0;
        $ignorados  = []; // XMLs duplicados que serão ignorados
        $erros      = []; // XMLs com erro no processamento
        ...
        foreach ($filesList as $file) {
            ...
            $nomeArquivo = $file->getClientName();

            try {
                $parsed = $this->parseNFe($xmlStr);
                if (!$parsed || empty($parsed['itens'])) {
                    $erros[] = $nomeArquivo . ' (XML sem itens válidos)';
                    continue;
                }
                ...
                if ($chave) {
                    ...
                    if ($duplicada) {
                        // NF-e já foi importada anteriormente
                        $infoNF = "NF {$duplicada['numero']}/{$duplicada['serie']}";
                        if (!empty($duplicada['competencia'])) {
                            $infoNF .= " (Competência: {$duplicada['competencia']})";
                        }
                        if (!empty($duplicada['empresa_nome'])) {
                            $infoNF .= " ({$duplicada['empresa_nome']})";
                        }
                        $ignorados[] = $nomeArquivo . ' - ' . $infoNF;
                        log_message('info', "NFe duplicada ignorada: {$nomeArquivo} - Chave: {$chave}");
                        continue;
                    }
                }

                // Processar NF-e (não é duplicada)
                ...
                $erros[] = $nomeArquivo . ' (' . $e->getMessage() . ')';
                log_message('error', 'Erro parse NFe '.$nomeArquivo.' => '.$e->getMessage());
            }
        }
        ...
        $mensagens = [];
        if ($countXML > 0) {
            $mensagens[] = "✅ {$countXML} XML(s) importado(s) com sucesso ({$countItens} item(ns))";
        }
        if (!empty($ignorados)) {
            $mensagens[] = "⚠️ " . count($ignorados) . " XML(s) ignorado(s) (já importados anteriormente)";
        }
        if (!empty($erros)) {
            $mensagens[] = "❌ " . count($erros) . " XML(s) com erro";
        }

        $mensagemFinal = "Lote #{$importId}: " . implode('. ', $mensagens);

        // Se tiver ignorados ou erros, adicionar detalhes ao flash
        if (!empty($ignorados)) {
            session()->setFlashdata('info_ignorados', $ignorados);
        }
        if (!empty($erros)) {
            session()->setFlashdata('info_erros', $erros);
        }

        return redirect()
            ->to(site_url(self::BASE.'/review/'.$importId))
            ->with('ok', $mensagemFinal);
    }
    ...
    public function review($importId)
    {
        ...
        // Buscar associações salvas (DE-PARA automático)
        $codigosProdutos = array_unique(array_filter(array_column($items, 'cProd')));
        $associacoes = $model->getAssociacoesProdutoServico($codigosProdutos);
        ...
        $data = [
            ...
            'empresas'    => $empresas,
            'servicos'    => $servicos,
            'associacoes' => $associacoes, // [codigo_produto => servico_id]
        ];
    ...
    public function finalize($importId)
    {
        ...
        $itensMes     = $this->request->getPost('itens_mes') ?? [];       // [itemId => mes]
        $itensAno     = $this->request->getPost('itens_ano') ?? [];       // [itemId => ano]
        ...
        foreach ($items as $it) {
            ...
            // Pega mês e ano do item específico
            $mes = (string) ($itensMes[$itemId] ?? '');
            $ano = (string) ($itensAno[$itemId] ?? '');

            if (empty($mes) || empty($ano)) {
                // Se não informou mês/ano, ignora esse item
                continue;
            }
            ...
            if ($ok) {
                ...
                // Salvar associação produto-serviço para próximas importações
                $cProd = $it['cProd'] ?? '';
                $xProd = $it['xProd'] ?? '';
                if (!empty($cProd)) {
                    $model->salvarAssociacaoProdutoServico($cProd, $xProd, $sid, $servNome);
                }
            }
        }

        // Atualiza cabeçalho do lote (empresa e status) - competência agora é por item
        $status = $inseridos > 0 ? 'FINALIZADO' : 'PARCIAL';
        $model->set_associations_header((int)$importId, null, $empresaNome, $status);
