...
class Despesas extends OperacionalBase
{
    ...
    public function index()
    {
        ...
        $filtroMes  = $this->request->getGet('filtro_mes') ?? '';
        $filtroAno  = $this->request->getGet('filtro_ano') ?? '';

        // Construir query com filtros
        $builder = $db->table('despesas');

        if (!empty($filtroTipo)) {
            $builder->where('tipo', $filtroTipo);
        }
        if (!empty($filtroMes)) {
            $builder->where('mes', (int)$filtroMes);
        }
        if (!empty($filtroAno)) {
            $builder->where('ano', (int)$filtroAno);
        }
        ...
        return $this->renderView('operacional/despesas/form', [
            ...
            'action'     => site_url('operacional/despesas/salvar'),
    ...
    {
        $db = \Config\Database::connect();
        $data = $this->request->getPost(['tipo','mes','ano','valor']);
    ...
    public function editar($id = null)
    {
        ...
        return $this->renderView('operacional/despesas/form', [
            ...
            'ultimas'    => $ultimas,
            'item'       => $item,
            'action'     => site_url('operacional/despesas/atualizar/'.$id),
            'filtroTipo' => '',
            'filtroMes'  => '',
            'filtroAno'  => '',
        ]);
